...
module Db
  ...
  def self.register_user(telegram_id : Int64)
    connection.exec("INSERT INTO users (telegram_id) VALUES ($1) ON CONFLICT (telegram_id) DO NOTHING", telegram_id)
  end

  def self.find_user_by_telegram(telegram_id : Int64)
    connection.query_one?("SELECT id FROM users WHERE telegram_id = $1", telegram_id, as: Int32)
  end
